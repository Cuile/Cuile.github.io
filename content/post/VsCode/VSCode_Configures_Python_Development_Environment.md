...
## 配置 Micromamba
[Micromamba的安装与使用]({{< relref "post/Python/micromamba.md">}})
...
### 创建 Python 项目环境
- 使用项目终端界面，[创建虚拟环境]({{< relref "post/Python/micromamba.md#3-创建项目">}})。
- 重启 vscode 再次进入项目终端，不一定会看到项目前缀[^2]。VsCode已经自动修你激活了Python虚拟环境，不需要再操作。
- 安装包时，不需要激活 micromamba 环境，否则包将被安装到 micromamba 环境中。
